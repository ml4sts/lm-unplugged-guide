...
We tend to use n as our window size for helping us decide on our next best word. This is known as an n-gram model. In more recent times, we use more advanced methods than windowing and at much _larger scales_, hence **large language models** (LLMs). 

###Tokenization & Embeddings
A document is a set of words/terms that express a coherent idea or topic. **Tokenization** is the process of breaking down/splitting these words into units (e.g., words, subwords, characters). These breaks can occur in a set of various ways, for example, when white space occurs. Once the words are broken down, they can be represented to computers in a digestible way called **embeddings**. When words are embedded, they are represented by a vector of numbers which maintains the semantics of the word. These words lay in a high-dimensional space (e.g., $\ R^{50}, R^{1000}$) where vectors (i.e., words) that are similar in meaning are near each other (e.g., _opportunity(v) and chance(w)_ are words that $\lvert \vec{v} - \vec{w} \rvert < \epsilon$, where $\\epsilon$ is small). Punctuation marks can also be represented through embeddings and are often counted as separate tokens. 


### [Transformers] 
LLMs use transformers, which dynamically determine the importance of each token within a fixed context window, limited by the model. 

First, a transformer takes in the word embeddings and sends them to the **attention layer**. The attention layer assigns weights to each input token embedding, reflecting its relevance to the current token of concern, and combines the corresponding value vectors to reveal the importance in context, given the information from the surrounding embeddings and the model's **parameters**. Next, the attention-weight adjusted embeddings are passed through a neural network (NN). The NN updates the weights on the embeddings and its parameters. This repeats between other attention layers and NNs until we pass through all of the layers in our model. _Backpropegation_ occurs to recalculate the weights given the error. That is, when our model fits a function (e.g., $\ y = mx + b$), the predicted output \hat{y} is tested against the actual outcome y, ($\ L_1 = |\hat{y} - y| < \epsilon$), if the $\ L_1 !< \epsilon$ we pass the loss back through the network to caculate the affect of the loss on each parameter ($\\theta_i$) (i.e., $\ \frac{dL}{d\theta_i}$). By **gradient descent** each parameter ($\\theta_i$) is updated: ($\\theta_i \leftarrow \theta_i - \eta * \frac{dL}{d\theta_i}$) (\eta is our learning rate that helps us decide how large of a step to take into our descent). All of this repeats until our loss no longer _desends_/decreases, we stop early given a number of epochs/runs, or until we reach a _sufficent_ model performance. 

### [Output]
Once we reach our goal, the last layer produces distributions/probabilities that describe how likely it is that the word represented by each embedding is to be the _next_ word in our sequence. The top word is likely to be chosen. However, depending on the parameter settings, we could choose to sometimes predict/guess the 2nd, 3rd, or other best (top_k). This is sometimes a strategic move that can help with creativity/strictness and accuracy. The word we choose as _next_ is now added to the document and additional words are found if we have not reached the [EOF] (i.e., end of file).
